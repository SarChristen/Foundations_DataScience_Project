...
GenesAB <- as.data.frame(GenesAB) # needed to be converted into data frame? (https://www.geeksforgeeks.org/r-language/convert-matrix-to-dataframe-in-r/)
...
Gene_Expression_Scatterplot <- ggplot(GenesAB, aes(x = ENSG00000007202.15, y = ENSG00000001631.16)) +
  ...
  annotate("text", x = 100000, y = 100, label = eq_label,
           hjust = 1.1, vjust = -1.1, size = 5, color = "red") + # what do hjust and vjust do? defines location of text on plot #(https://www.geeksforgeeks.org/r-language/what-do-hjust-and-vjust-do-when-making-a-plot-using-ggplot/)
  theme_linedraw()
...
#info for adding regression line to ggplot2 plot (https://www.geeksforgeeks.org/r-language/add-regression-line-to-ggplot2-plot-in-r/)
#also (https://www.statology.org/add-regression-equation-to-plot-in-r/)
